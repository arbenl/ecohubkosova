...
export const ecoOrganizations = pgTable("eco_organizations", {
  ...
  // Metadata
  // IMPORTANT: Use safeJsonb() instead of jsonb() to prevent double-parse crashes.
  // postgres-js auto-parses JSONB columns into objects, but Drizzle's jsonb() tries
  // to JSON.parse() again, causing "SyntaxError at position 1034". safeJsonb() detects
  // pre-parsed objects and handles them safely. See docs/db-jsonb-safety.md
  metadata: safeJsonb("metadata").default(sql`'{}'::jsonb`),
...
export const ecoListings = pgTable(
  "eco_listings",
  {
    ...
    // Metadata
    // IMPORTANT: Use safeJsonb() - see comment at ecoOrganizations.metadata above
    metadata: safeJsonb("metadata").default(sql`'{}'::jsonb`),
