...
test.describe("Albanian Home Page (/sq/home)", () => {
  ...
  test("should display Albanian navigation links", async ({ page }) => {
    ...
    // Verify Albanian navigation text (actual header links)
    await expect(page.getByText("Tregu")).toBeVisible() // Marketplace
    await expect(page.getByText("Partnerët")).toBeVisible()
    await expect(page.getByText("Si funksion")).toBeVisible() // How it works
    await expect(page.getByText("Rreth nesh")).toBeVisible() // About
