...
export function PartnersClient({ locale, partners }) {
  ...
  return (
    <div className="min-h-screen space-y-10 pb-16 bg-white">
      ...
      <div className="mx-auto max-w-6xl space-y-8">{/* Filter buttons, partner grid, etc */}</div>
    </div>
  )
}
```
...
```jsx
<PublicPageHero namespace="partners" titleKey="page.title" subtitleKey="page.subtitle" />
```
...
```jsx
<PublicPageHero variant="centered" {...props} />
```
