...
password: z.string({ required_error: "Fjalëkalimi është i detyrueshëm." }).min(
  6,
...
export async function middleware(req: NextRequest) {
  if (
    req.method === "POST" &&
    !CSRF_EXEMPT_ROUTES.some((r) => req.nextUrl.pathname.startsWith(r))
  ) {
...
export async function validateOAuthState(state: string, storedState: string): Promise<boolean> {
