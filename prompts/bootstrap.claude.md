...
- **Context & Docs/Tests:** Use **context7** for semantic context, `mcp-db-schema` + `mcp-db-inspect` (mapped to context7) for schema/data, `mcp-docs-knowledge` + **markitdown** for docs (mapped to context7/markitdown), **playwright** for browser/E2E, `mcp-test-runner` (mapped to ecohub-qa) for validation, and `mcp-ux-assets` (mapped to context7) when touching UI. Check `mcp-outputs/mcp-health.json` for availability.
