...
Confirm MCP servers are running via `mcp-outputs/mcp-health.json` or the CLI output, then proceed with MCP tools/logs.
...
1. **Ask me to provide updated logs and docs:**
   ...
   - Ask for `mcp-outputs/mcp-health.json` to see which MCP servers are available
